class Character < ActiveRecord::Base
  ...
  def total_xp
    total = log_entries.pluck(:xp_gained).inject(:+)
  ...
  def total_gp
    log_entries.pluck(:gp_gained).inject(:+)
  end

  def total_renown
    log_entries.pluck(:renown_gained).inject(:+)
  end

  def total_downtime
    log_entries.pluck(:downtime_gained).inject(:+)
  end

  def total_magic_items
    log_entries.pluck(:num_magic_items_gained).inject(:+)
  end

  def magic_items_list
    log_entries.pluck(:desc_magic_items_gained).delete_if{ |x| x == "" }.join(',')
  end
end
